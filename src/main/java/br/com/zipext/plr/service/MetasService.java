...
public class MetasService {
	...
	public ByteArrayInputStream export(TempoModel periodo) throws IOException {
		XlsFileExport export = new XlsFileExport(
				this.propertyService.getProperty(EnumProperty.XLS_TEMPLATE_INDICADORES_PATH),
				EnumXLSSheets.INDICADORES);
	...
	public List<MetasModel> findAllAtivasByOrderByDescricaoAsc() {
		return this.repository.findAllAtivasByOrderByDescricaoAsc(EnumSituacao.ATIVO.getCodigo().toString());
	}
	...
	public List<MetasModel> findByDescricaoAndSituacao(String descricao, String situacao) {
		return this.repository.findByDescricaoAndSituacao(descricao, situacao);
	}

	@Transactional(readOnly = true)
	public List<MetasModel> findByFilter(MetasModel model, String codigoLegado, String meta, String situacao,
			TipoMedicaoModel tipoMedicao, TipoMetaModel tipoMeta, FormulaModel formula,
			FrequenciaMedicaoModel frequenciaMedicao) {
		return this.repository.findByFilter(model, codigoLegado, meta, situacao, tipoMedicao, tipoMeta, formula,
				frequenciaMedicao);
	}
	...
	@Transactional(readOnly = true)
	public List<MetasModel> findProjetosVencidosByResponsavel(Integer periodoPLR, Long skyDataLimite,
			ColaboradorModel aprovador) {
		return this.repository.findProjetosVencidosByResponsavel(periodoPLR, skyDataLimite, aprovador,
				Arrays.asList(EnumTipoMeta.PROJETO.name(), EnumTipoMeta.ENTREGA.name()));
	}

	@Transactional(readOnly = true)
	public List<MetasModel> findQualitativas() {
		return this.repository.findByQualificadorMeta(EnumQuantQual.QUALITATIVA.getCodigo().toString());
	}

	@Transactional(readOnly = true)
	public List<MetasModel> findQuantitativas() {
		return this.repository.findByQualificadorMeta(EnumQuantQual.QUANTITATIVA.getCodigo().toString());
	}
